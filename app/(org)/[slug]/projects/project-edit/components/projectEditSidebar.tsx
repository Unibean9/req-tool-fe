...
export type ProjectEditSidebarProps = {
    ...
    >
      ...
      >
        <ol
          ...
          style={{
            gridTemplateRows: `repeat(${PROJECT_NEW_STEPS.length}, minmax(0, 1fr))`,
          }}
        >
          {PROJECT_NEW_STEPS.map((step, index) => {
            const isActive = index === currentStepIndex;
            const isComplete = freeNavigation
              ? !isActive
              : index < currentStepIndex;
            const isPending = freeNavigation ? false : index > currentStepIndex;
            const canSelect = freeNavigation ? true : index <= currentStepIndex;
