...
export default function UseCaseScreen() {
  ...
  }, [edges, nodes, response?.projectName, plan?.systemBoundary, lineType]);
  ...
  return <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto pb-4">
    ...
    </div> : <section ref={diagramSectionRef} className={`flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-border/70 bg-card/25 ${isDiagramFullscreen ? "h-screen w-screen rounded-none bg-background p-2" : ""}`}><div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/70 px-4 py-3"><div><h2 className="text-sm font-semibold">{plan?.systemBoundary ?? response.projectName}</h2><p className="mt-1 text-xs text-muted-foreground">React Flow preview · {edges.length} relationships</p></div><div className="flex flex-wrap items-center gap-2"><select aria-label="Diagram" value={plan?.diagramId ?? ""} onChange={(e) => setSelectedPlanId(e.target.value)} className="h-8 max-w-60 rounded-md border border-border/70 bg-background px-2 text-xs">{response.diagramPlans.map((item) => <option key={item.diagramId} value={item.diagramId}>{item.level} · {item.systemBoundary}</option>)}</select><select aria-label="Connector line style" className="h-8 rounded-md border border-border/70 bg-background px-2 text-[10px]" onChange={(event) => setLineType(event.target.value as typeof lineType)}><option value="straight">Straight line</option><option value="step">Right angle</option><option value="smoothstep">Rounded elbow</option><option value="default">Bezier curve</option></select><select aria-label="Relationship type" className="h-8 rounded-md border border-border/70 bg-background px-2 text-[10px]" onChange={(event) => setRelationship(event.target.value as typeof relationship)}><option value="association">Association</option><option value="include">«include»</option><option value="extend">«extend»</option><option value="generalization">Generalization</option></select>{selectedCount > 0 ? <button type="button" disabled={saving} onClick={deleteSelected} className="inline-flex h-8 items-center gap-1.5 rounded-md border border-destructive/40 px-2.5 text-[10px] text-destructive hover:bg-destructive/10"><Trash2 className="size-3.5" />Delete selected · {selectedCount}</button> : null}<button type="button" disabled={!plan} onClick={() => void exportDiagram().catch((error) => setUiError(getApiErrorMessage(error, "Could not export diagram.")))} className="inline-flex h-8 items-center gap-1.5 rounded-md border border-border/70 px-2.5 text-[10px] hover:bg-muted/50"><Download className="size-3.5" />Export PNG</button><button type="button" onClick={() => void toggleDiagramFullscreen().catch((error) => setUiError(getApiErrorMessage(error, "Full screen is unavailable.")))} className="inline-flex h-8 items-center gap-1.5 rounded-md border border-border/70 px-2.5 text-[10px] hover:bg-muted/50">{isDiagramFullscreen ? <Minimize2 className="size-3.5" /> : <Maximize2 className="size-3.5" />}{isDiagramFullscreen ? "Exit full screen" : "Full screen"}</button></div></div>
