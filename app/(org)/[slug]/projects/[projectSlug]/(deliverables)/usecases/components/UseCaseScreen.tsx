...
export default function UseCaseScreen() {
  ...
  return <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto pb-4 [&>header]:shrink-0 [&>details]:shrink-0 [&>p]:shrink-0">
    <header className="rounded-xl border border-border/70 bg-card/35 p-4">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-3"><h1 className="text-3xl font-semibold tracking-tight">Use case model</h1><span className="inline-flex items-center gap-1 rounded-full border border-amber-500/40 bg-amber-500/10 px-2.5 py-1 text-[10px] font-medium text-amber-200"><Sparkles className="size-3" />{groupProgress ? `Generating group ${groupProgress.current}/${groupProgress.total}…` : busy ? "Saving…" : "Project model"}</span></div>
    ...
    {groupProgress ? <p role="status" className="rounded-lg border border-primary/30 bg-primary/5 p-3 text-xs text-primary">Generating group {groupProgress.current} of {groupProgress.total}: {groupProgress.groupName}…</p> : null}
